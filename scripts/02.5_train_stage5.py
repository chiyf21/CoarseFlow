# Stage 5: Spatial residual refinement — freeze coarse, train residual only
...
def main():
    ...
    if is_main:
        print("=" * 80)
        print("[Stage 5] Spatial residual refinement — freeze all except residual")
    ...
    # ------------------------------------------------------------
    # Keep the coarse matcher at one refinement iteration to avoid
    ...
    model_config = dict(
        ...
        # matching on /8 XY features, so allow several raw-pixel XY
        ...
        # a lightweight calibrator on top of the trained coarse model.
    ...
    if is_main:
        print("[Done] Stage 5 spatial residual refinement finished.")
